...
namespace BAL
{
    ...
    public class Cand_ShortlistingDetail : Abstracttest<HrShortlistingDetail>
    {
        private DbSaveClass<HrShortlistingDetail> repository = null;
        public Cand_ShortlistingDetail()
        {
            this.repository = new DbSaveClass<HrShortlistingDetail>();

        }
        public override List<HrShortlistingDetail> GetList(int ShortListId)
        {
            var modeldata = repository.GetAllRecord();
            return modeldata.Where(m => m.ShortListId == ShortListId).ToList();
        }

   

        public HrShortlistingDetail GetById(int ShortListId)
        {
            var modeldata = repository.GetAllRecord();
            return modeldata.Where(m => m.ShortListId == ShortListId).FirstOrDefault();
        }
        public override bool AddDatabool(HrShortlistingDetail model)
        {
            //if (model.UserId == null || model.UserId == 0)
            //{
            //    model.UserId = GlobalUserInfo.UserId;
        ...
        public bool AddForeach(List<HrShortlistingDetail> modelList)
        {
            //var recordmodel = GetList(GlobalUserInfo.UserId);
            //repository.DeleteRange(recordmodel);
            //repository.Save();
